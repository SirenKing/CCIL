...
function DoInventoryStuff() {
    var hud = GetDotaHud();
    var inventoryPanels = hud.FindChildTraverse("inventory");
    if (!inventoryPanels) {
        $.Schedule(1, DoInventoryStuff);
        return;
    }

    for (let i=0;i<=8;i++) {
        var inventorySlotPanel = inventoryPanels.FindChildTraverse("inventory_slot_"+i);
        var inventorySlotButton = inventorySlotPanel.FindChildTraverse("AbilityButton");
        inventorySlotButton.SetPanelEvent("onmouseover", function Miaow() {
		var hero = Players.GetPlayerHeroEntityIndex( Players.GetLocalPlayer() );
        $.Msg("Hero is ",hero);
		var item = Entities.GetItemInSlot( hero, i );
        $.Msg("Item ", item, " is in slot ", +i);
        var itemNameString = Abilities.GetAbilityName( item );
        $.Msg("Item name is ", itemNameString);
        $.Msg("Item index is ", item);
		GetAttributesPanelInventory(item);
        });

        inventorySlotButton.SetPanelEvent("onmouseout", function Clear() {
				var hero = Players.GetPlayerHeroEntityIndex( Players.GetLocalPlayer() );
        });
    }
}
...
function GetAttributesPanelInventory(item) {
