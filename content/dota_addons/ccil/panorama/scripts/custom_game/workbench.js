...
GameEvents.Subscribe("game_init", function() {

  var hud = GetDotaHud();
  var workbenchPanel = hud.FindChildTraverse("WorkbenchPanel");

  if( workbenchPanel.visible == true) {
...
GameEvents.Subscribe("dota_player_update_query_unit", function() {
	$.Msg("Player selected a unit!");
	var unit = Players.GetQueryUnit( Players.GetLocalPlayer() );
	var unitName = Entities.GetUnitName( unit );
	$.Msg( "Player selected  " + unitName );

  var hud = GetDotaHud();
  var workbenchPanel = hud.FindChildTraverse("WorkbenchPanel");

  if(unitName == "npc_dota_workbench") {
      workbenchPanel.visible = true;
...
function ClonePlayerInventory() {
	...
	for (i = 0; i < 6; i++) {
		...
		var itemTexture = Items.GetAbilityTextureSF( item );
		var itemTexturePath = "file://{images}/items/"+itemTexture+".png"
		var itemTextureValue = "url('" + itemTexturePath + "')"
		$.Msg(itemTexturePath);

	  var hud = GetDotaHud();
		var workbenchPanel = hud.FindChildTraverse("WorkbenchPanel");
		var slotClone = workbenchPanel.FindChildTraverse("itemSlot"+i);
		slotClone.style.backgroundImage = itemTextureValue;
		slotClone.style.backgroundSize = "60px 45px";
		}
};

function OnSelectedItem( arg ) {
	$.Msg("Player is has selected bench slot " + arg )
	var player = Players.GetLocalPlayer();
	var hero = Players.GetPlayerHeroEntityIndex( player );
	var item = Entities.GetItemInSlot( hero, arg );
	ChangeAttributesText(item);
	var itemSlotToRoll = arg;
};
...
function ChangeAttributesText(item) {

	var hud = GetDotaHud();
	var player = Players.GetLocalPlayer();
	var hero = Players.GetPlayerHeroEntityIndex( player );
	var radios = hud.FindChildrenWithClassTraverse('inventorySlotCopy');
	var slotID = 0;
	...
	var itemSlot = slotID.slice(-1);
	var item = Entities.GetItemInSlot( hero, parseInt(itemSlot) );

	var workbenchPanel = hud.FindChildTraverse('WorkbenchPanel');
	var abilityAttributes = workbenchPanel.FindChildTraverse('attributesTextPanel');
	...
	abilityAttributes.html = true
	abilityAttributes.text = extraTextInventory.join('<br>');

	return abilityAttributes;
}

function RollAllMods() {
	$.Msg("Player decided to roll all mods on item ");
	var hud = GetDotaHud();
	var player = Players.GetLocalPlayer();
	var hero = Players.GetPlayerHeroEntityIndex( player );
	var gold = Players.GetGold( player );
	var bench = Players.GetQueryUnit( Players.GetLocalPlayer() );
	var radios = hud.FindChildrenWithClassTraverse('inventorySlotCopy');
	var slotID = 0;
	...
	var itemSlot = slotID.slice(-1);
	$.Msg("Turns out that item " + itemSlot + " has been selected");
	var item = Entities.GetItemInSlot( hero, parseInt(itemSlot) );

	GameEvents.SendCustomGameEventToServer( "roll_mods", { "player" : player, "item_index" : item, "roll_type" : "all_mods", "gold_cost" : 400, "workbench" : bench, } );
	$.Schedule( 0.1, ChangeAttributesText );
}
