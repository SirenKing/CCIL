if modifier_tower_may_cast_spells == nil then
    modifier_tower_may_cast_spells = class({})
end

function modifier_tower_may_cast_spells:OnCreated(keys)
    ...
    self.args.procChance  = .75
    self.args.problematicChance  = .30
    self.trolls = {}
...
function modifier_tower_may_cast_spells:DebugAbility(sAbility)
...
function modifier_tower_may_cast_spells:DebugUnit()
...
function modifier_tower_may_cast_spells:SendDebugNote(header,content,time)
    CustomGameEventManager:Send_ServerToAllClients("debug_notifications",{header=header,content=content,time=time})
end

function modifier_tower_may_cast_spells:DeclareFunctions()
    return    {
...
function modifier_tower_may_cast_spells:CheckCreep(npc)
	local name = npc:GetUnitName()
	if (name ~= nil and string.find(name,"npc_dota_creep_") ~= nil) then return true end
	return false
end
function modifier_tower_may_cast_spells:OnDeath(keys)
    if not IsServer() then return end
...
function modifier_tower_may_cast_spells:OnAbilityExecuted(keys)
    if not IsServer() then return end
    if keys.unit ~= self:GetParent() then return end
...
function modifier_tower_may_cast_spells:GetRandomSpell()
    local hHero = self:GetParent()
    local tPossible = {}
    for i = 0, hHero:GetAbilityCount()-1 do
...
function modifier_tower_may_cast_spells:CheckItem(hAbility)
...
function modifier_tower_may_cast_spells:GetLastSpell()
...
function modifier_tower_may_cast_spells:CastASpell(hTarget)
...
function modifier_tower_may_cast_spells:CheckOption(op)
    return (op == 1 or op == true)
end

function modifier_tower_may_cast_spells:OnAttackLanded(keys)
    if not IsServer() then return end
...
function modifier_tower_may_cast_spells:FlagCheck(iFlags,i)
    return (bit.band(iFlags,i) == i)
end

function modifier_tower_may_cast_spells:FlagFilter(nTeam,nFlags,hTarget,hHero)
...
function modifier_tower_may_cast_spells:FindStrictTarget(hAbility,hcTarget)
    local hHero = self:GetParent()
    local nTeam = hAbility:GetAbilityTargetTeam() or DOTA_UNIT_TARGET_TEAM_BOTH
    local nFlags = hAbility:GetAbilityTargetType() or DOTA_UNIT_TARGET_ALL
    local nBehav = hAbility:GetBehavior()
...
function modifier_tower_may_cast_spells:FindTarget(nTeam,nFlags,hcTarget,hAbility)
    local hHero = self:GetParent()
    local nTeam = hAbility:GetAbilityTargetTeam() or DOTA_UNIT_TARGET_TEAM_BOTH
    local nBehav = hAbility:GetBehavior()
...
function modifier_tower_may_cast_spells:IsDebuff() return false end
function modifier_tower_may_cast_spells:DestroyOnExpire() return false end
function modifier_tower_may_cast_spells:IsDebuff() return false end
function modifier_tower_may_cast_spells:IsHidden() return true end
function modifier_tower_may_cast_spells:IsPermanent() return true end
function modifier_tower_may_cast_spells:IsPurgable() return false end
function modifier_tower_may_cast_spells:IsPurgeException() return false end
function modifier_tower_may_cast_spells:IsStunDebuff() return false end
function modifier_tower_may_cast_spells:AllowIllusionDuplicate() return false end
function modifier_tower_may_cast_spells:GetAttributes() return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE + MODIFIER_ATTRIBUTE_PERMANENT end


function modifier_tower_may_cast_spells:CheckTrollAbility(hAbility)
...
function modifier_tower_may_cast_spells:CheckProblematic(hAbility)
    local name = hAbility:GetAbilityName()
...
function modifier_tower_may_cast_spells:InList(list,hAbility)
    local name = hAbility:GetAbilityName()
...
function modifier_tower_may_cast_spells:DebugStuff(hAbility)
